...
-- ============================================================
-- Run this AFTER sql/stage_c2a_rpcs.sql has been applied successfully.
...
--
-- Note on tests 3/4 below: can_submit_picks's Official result depends on
...
--   - Test 6 inserts one real `picks` row (your player, Casual bracket, the
...
--     else in this file writes anything (tests 1-5 are all reads).
...
begin
  ...
  if v_league_id is null then
    ...
    -- 2. get_bracket_seasons doesn't error (0 rows is a valid, expected answer post-wipe)
    begin
      select count(*) into v_rows from get_bracket_seasons(v_official_id);
      insert into smoke_results_c2a values (2, 'get_bracket_seasons callable', 'PASS', format('%s season(s)', v_rows));
    exception when others then
      insert into smoke_results_c2a values (2, 'get_bracket_seasons callable', 'FAIL', sqlerrm);
    end;

    -- 3. my_leagues returns the new official_opt_in column, non-null
    begin
      select official_opt_in into v_opt_in from my_leagues(v_name, v_pin) where bracket_id = v_casual_id;
      insert into smoke_results_c2a values (3, 'my_leagues returns official_opt_in', case when v_opt_in is not null then 'PASS' else 'FAIL' end,
        format('official_opt_in=%s', v_opt_in));
    exception when others then
      insert into smoke_results_c2a values (3, 'my_leagues returns official_opt_in', 'FAIL', sqlerrm);
    end;

    -- 4. can_submit_picks — Casual should always be ok=true
    begin
      select * into v_gate from can_submit_picks(v_name, v_pin, v_casual_id, v_show_id);
      insert into smoke_results_c2a values (4, 'can_submit_picks: Casual', case when v_gate.ok then 'PASS' else 'FAIL' end,
        format('ok=%s reason=%s', v_gate.ok, v_gate.reason));
    exception when others then
      insert into smoke_results_c2a values (4, 'can_submit_picks: Casual', 'FAIL', sqlerrm);
    end;

    -- 5. can_submit_picks — Official: reports the actual result (see header note)
    begin
      select * into v_gate from can_submit_picks(v_name, v_pin, v_official_id, v_show_id);
      insert into smoke_results_c2a values (5, 'can_submit_picks: Official', 'INFO',
        format('ok=%s reason=%s', v_gate.ok, v_gate.reason));
    exception when others then
      insert into smoke_results_c2a values (5, 'can_submit_picks: Official', 'FAIL', sqlerrm);
    end;

    -- 6. submit_picks to Casual still works after the body re-touch (writes a real pick row, cleaned up below)
    begin
      perform submit_picks(v_name, v_pin, v_casual_id, v_show_id,
        '[{"slot":"opener","songname":"Test Song"}]'::jsonb);
      insert into smoke_results_c2a values (6, 'submit_picks to Casual still succeeds', 'PASS', null);
    exception when others then
      insert into smoke_results_c2a values (6, 'submit_picks to Casual still succeeds', 'FAIL', sqlerrm);
    end;
    ...
    insert into smoke_results_c2a values (7, 'cleanup', 'PASS', 'removed the test pick row');
  end if;
end $$;
